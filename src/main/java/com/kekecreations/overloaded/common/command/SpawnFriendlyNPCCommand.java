...
public class SpawnFriendlyNPCCommand extends AbstractTargetPlayerCommand {

    private final RequiredArg<String> npcId;



    public SpawnFriendlyNPCCommand() {
        super("spawn_friendly", "Spawning NPC Command for Overloaded!");
    ...
    protected void execute(@NotNull CommandContext commandContext, @Nullable Ref<EntityStore> ref, @NotNull Ref<EntityStore> ref1, @NotNull PlayerRef playerRef, @NotNull World world, @NotNull Store<EntityStore> store) {
        ...
        double negativeX = -1 * (Math.random() * 4);
        double positiveX =  (Math.random() * 4);
        double negativeZ = -1 * (Math.random() * 4);
        double positiveZ =  (Math.random() * 4);
        if (player != null) {
